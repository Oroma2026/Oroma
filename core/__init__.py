...
import logging
# NOTE:
# Importing log_guard at module import time must never break UI startup.
# If log_guard is temporarily in a bad state (partial init / local edits),
# we fall back to a no-op to avoid taking the whole service down.
try:
    from core.log_guard import log_suppressed
except Exception:  # pragma: no cover
    def log_suppressed(*args, **kwargs):  # type: ignore
        return None
if TYPE_CHECKING:
