...
#   OROMA_PTZ_MOTOR_POLICY_POS_THR=0.02      (Default: 0.02)
#   OROMA_PTZ_MOTOR_POLICY_NEG_THR=-0.02     (Default: -0.02)
#   OROMA_PTZ_MOTOR_POLICY_NS=ptz_motor      (Default: ptz_motor)
#
...
class DreamWorker(threading.Thread):
    ...
    def _phase_defs(self) -> List[tuple[str, Any, bool]]:
        return [
            ...
            ("ptz_policy_motor", self._ptz_policy_from_motor_utility, True),
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if self.interval == 0:
                ...
                # ------------------------------------------------------------------
                # Optionaler gezielter Single-Phase-Lauf
                # ------------------------------------------------------------------
                # Hintergrund:
                #   In grossen ORÓMA-Instanzen kann die Kernphase "replay" das
                #   komplette Single-Run-Budget verbrauchen. Additive, schnelle
                #   Dream-Verdichtungen wie "ptz_policy_motor" waeren dann zwar
                #   korrekt registriert, kaemen in manuellen Tests aber nie zur
                #   Ausfuehrung.
                #
                #   OROMA_DREAM_ONLY_PHASES erlaubt deshalb einen expliziten,
                #   diagnostischen/operativen Lauf einzelner Phasen ohne vorherige
                #   Kernphasen. Das ist bewusst allgemein gehalten und enthaelt
                #   keine PTZ-Sonderlogik. Mehrere Phasen koennen kommasepariert
                #   angegeben werden. Ungueltige Namen werden sichtbar geloggt und
                #   ignoriert.
                #
                # Beispiele:
                #   OROMA_DREAM_ONLY_PHASES=ptz_policy_motor python3 core/dream_worker.py
                #   python3 core/dream_worker.py --phase ptz_policy_motor
                # ------------------------------------------------------------------
                ...
                if forced_phase_raw:
                    for item in forced_phase_raw.replace(";", ",").split(","):
                        name = str(item or "").strip()
                        if not name:
                            continue
                        if name not in all_phase_map:
                            LOG.warning("DreamWorker: unbekannte OROMA_DREAM_ONLY_PHASES-Phase ignoriert: %s", name)
                            continue
                        if name not in forced_phase_names:
                            forced_phase_names.append(name)
                    if forced_phase_names:
                        core_phase_defs = []
                        selected_phase_map = all_phase_map
                ...
                selected: List[str] = []
                if forced_phase_names:
                    selected = list(forced_phase_names)
                    next_idx = idx
                    mode = "only"
                elif mode == "full":
                ...
                for name, fn in core_phase_defs:
                    ...
                    for name in selected:
                        if _budget_reached():
                            ...
                            break
                        fn = selected_phase_map.get(name)
                        if fn is None:
                            LOG.warning("DreamWorker: geplante Phase ohne Callable uebersprungen: %s", name)
                            continue
                        if not _phase(name, fn):
                            state["run_status"] = "budget_hit"
                            self._save_dream_state(state)
            ...
            while not self._stop_event.is_set():
                ...
                _phase("ptz_policy_motion", self._ptz_policy_from_motion_focus)
                _phase("ptz_policy_audio", self._ptz_policy_from_audio_probe)
                _phase("ptz_policy_motor", self._ptz_policy_from_motor_utility)
    ...
    def _ptz_policy_from_motor_utility(self) -> None:
        """Aggregate PTZ motor UtilitySignals into policy_rules (namespace='ptz_motor').

        Zweck
        -----
        Patch 4 der Structured-Plasticity-Linie verdichtet die vom separaten
        `tools/ptz_motor_reward_collector.py` erzeugten Utility-Signale in
        `policy_rules`, ohne die Motorsteuerung selbst zu beeinflussen.

        Quelle
        ------
        `rewards_log.source LIKE 'ptz_motor/%'` mit Utility-RAW-Struktur:
            raw.utility == true
            raw.utility_version >= 1
            raw.context.policy_action != ""

        Warum `policy_action`?
        ----------------------
        Der Collector unterscheidet bewusst zwischen:
          - policy_action:  ausgeführte, policy-taugliche Motoraktion (nur cmd_ok=True)
          - proposed_action: berechneter/diagnostischer Vorschlag ohne Ausführung
          - executed_action: tatsächlich ausgeführte Worker-Aktion

        Nur Einträge mit nicht-leerer `policy_action` sind echte motorische
        Lernbelege. Beobachtungs-/Cooldown-/Hold-Signale mit leerer
        policy_action werden für spätere perzeptive Analyse aufbewahrt, aber hier
        nicht in motorische policy_rules übernommen.

        State-Hash
        ----------
        Der State wird domänenarm und float-schonend aus Kontextfeldern gebildet.
        Roh-Floats werden gebucketet, damit policy_rules nicht durch einzigartige
        Messwerte explodiert. Die Action selbst wird NICHT in den State-Hash
        aufgenommen, sondern bleibt die `action`-Spalte von policy_rules.

        ENV
        ---
        OROMA_PTZ_MOTOR_POLICY_DREAM_ENABLE (Default: 1)
        OROMA_PTZ_MOTOR_POLICY_MAX_ROWS     (Default: 500)
        OROMA_PTZ_MOTOR_POLICY_POS_THR      (Default: 0.02)
        OROMA_PTZ_MOTOR_POLICY_NEG_THR      (Default: -0.02)
        OROMA_PTZ_MOTOR_POLICY_NS           (Default: ptz_motor)
        """
        if not _env_bool("OROMA_PTZ_MOTOR_POLICY_DREAM_ENABLE", True):
            return

        namespace = str(os.environ.get("OROMA_PTZ_MOTOR_POLICY_NS", "ptz_motor") or "ptz_motor").strip() or "ptz_motor"
        max_rows = max(10, min(int(_env_int("OROMA_PTZ_MOTOR_POLICY_MAX_ROWS", 500)), 5000))
        pos_thr = _env_float("OROMA_PTZ_MOTOR_POLICY_POS_THR", 0.02)
        neg_thr = _env_float("OROMA_PTZ_MOTOR_POLICY_NEG_THR", -0.02)
        if neg_thr > 0:
            neg_thr = -abs(float(neg_thr))
        ...
        try:
            with sql_manager.get_conn() as conn:
                ...
                rows = conn.execute(
                    ...
                    FROM rewards_log
                    WHERE source LIKE ? AND id>?
                    ORDER BY id ASC
                    LIMIT ?
                    """,
                    ("ptz_motor/%", int(last_id), int(max_rows)),
                ).fetchall() or []
        except Exception as e:
            log_suppressed(_BOOT_LOG, key="dream_worker.ptz_motor_policy.fetch", msg="PTZ motor policy fetch skipped.", exc=e, level=logging.WARNING, interval_s=300)
            return

        if not rows:
            return

        def _parse_raw(raw_s: Any) -> Dict[str, Any]:
            try:
                j = json.loads(raw_s) if raw_s else {}
                return j if isinstance(j, dict) else {}
            except Exception:
        ...
        def _clean_token(value: Any, *, default: str = "") -> str:
            ...
            for ch in txt:
                if ch.isalnum() or ch in ("_", "-", "/", "."):
                    ...
                    out.append("_")
            return "".join(out)[:96] or default

        def _bucket_float(value: Any, *, step: float = 0.05, default: str = "na") -> str:
            try:
                v = float(value)
                if not (v == v) or v in (float("inf"), float("-inf")):
                    return default
                step_f = max(0.001, float(step))
                b = int(round(v / step_f))
                return f"b{b:+d}"
            except Exception:
                return default

        def _state_hash_for(source: str, ctx: Dict[str, Any]) -> str:
            payload = {
                "source": _clean_token(source, default="ptz_motor/unknown"),
                "reason": _clean_token(ctx.get("reason"), default="unknown"),
                "candidate_winner": _clean_token(ctx.get("candidate_winner"), default="none"),
                "candidate_source": _clean_token(ctx.get("candidate_source"), default="none"),
                "target_mode": _clean_token(ctx.get("target_mode"), default="unknown"),
                "target_update": _clean_token(ctx.get("target_update"), default="unknown"),
                "target_hold_active": 1 if bool(ctx.get("target_hold_active")) else 0,
                "eye_hold_bias_active": 1 if bool(ctx.get("eye_hold_bias_active")) else 0,
                "axis_lock_active": 1 if bool(ctx.get("axis_lock_active")) else 0,
                "before_dist_bucket": _bucket_float(ctx.get("before_dist"), step=0.05),
                "before_target_conf_bucket": _bucket_float(ctx.get("before_target_conf"), step=0.05),
            }
            blob = json.dumps(payload, sort_keys=True, ensure_ascii=False, separators=(",", ":"))
            return "ptz_motor:" + hashlib.sha1(blob.encode("utf-8", errors="replace")).hexdigest()[:20]
        ...
        try:
            ...
            for rid, created_at, source, reward, raw_s in rows:
                try:
                    rid_i = int(rid)
                    new_last_id = rid_i
                    r = float(reward)
                ...
                rawj = _parse_raw(raw_s)
                if rawj.get("utility") is not True:
                    skipped += 1
                    skipped_no_utility += 1
                    continue

                ctx = _as_dict(rawj.get("context"))
                action = _clean_token(ctx.get("policy_action"), default="")
                if not action:
                    skipped += 1
                    skipped_no_action += 1
                    continue

                source_s = str(source or rawj.get("source") or "ptz_motor/unknown")
                state_hash = _state_hash_for(source_s, ctx)
                ...
                ins_params = [namespace, state_hash, action, 0, 0, 0, 0, 0.0, ts1]
            ...
            ck_params = [ck_key, str(int(new_last_id))]
            if use_dbw:
                db_writer_client.exec_write(ck_sql, ck_params, tag="dream_worker.ptz_motor_policy.ck", priority="low", timeout_ms=2000, db="oroma")
            else:
                conn.execute(ck_sql, ck_params)
                conn.commit()
        except Exception as e:
            log_suppressed(_BOOT_LOG, key="dream_worker.ptz_motor_policy.upsert", msg="PTZ motor policy upsert skipped.", exc=e, level=logging.WARNING, interval_s=300)
            return
        finally:
            if conn is not None:
                try:
        ...
        try:
            self._metric_inc("ptz:motor_policy_updates", float(updated))
            self._metric_inc("ptz:motor_policy_skipped", float(skipped))
            self._metric_inc("ptz:motor_policy_skipped_no_action", float(skipped_no_action))
        except Exception:
            pass
...
def _parse_args():
    ...
    ap.add_argument("--once", action="store_true", help="Kompatibilitaetsalias fuer einen Single-Run; entspricht --interval 0")
...
if __name__ == "__main__":
    ...
    # CLI-Kompatibilitaet und gezielte Phasensteuerung fuer manuelle Tests.
    # --once ist bewusst ein Alias, damit bestehende iPhone/Terminal-Kommandos
    # nicht fehlschlagen, obwohl der Worker bereits per interval=0 einen
    # Single-Run ausfuehrt.
